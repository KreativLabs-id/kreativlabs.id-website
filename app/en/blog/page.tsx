          ...
          <AnimatedSection animation="fade-up">
            <div className="text-center max-w-2xl mx-auto">
              <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-foreground tracking-tight mb-3">
                Insights & <span className="text-primary">Digital Strategy</span>
              </h1>
              <p className="text-foreground/70 text-sm sm:text-base leading-relaxed">
                Practical guides and actionable insights on web engineering, modern design, SEO, and business growth.
              </p>
            </div>
          </AnimatedSection>
        </div>
        ...
        <div className="container mx-auto px-4 sm:px-6 max-w-6xl">
          ...
          {sortedPosts.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {sortedPosts.map((post, index) => (
                <FadeInStagger key={post.id} index={index} delay={40}>
                  <Link href={`/en/blog/${post.slug}`} className="block h-full">
            ...
            <div className="text-center py-16">
              <p className="text-foreground/60 text-base">No articles published yet.</p>
            </div>
          )}
          ...
          <AnimatedSection animation="fade-up" delay={150}>
            <div className="bg-card border border-border/80 rounded-2xl p-8 sm:p-12 text-center mt-16 max-w-4xl mx-auto">
              <h2 className="text-2xl sm:text-3xl font-bold text-foreground mb-3 tracking-tight">
                Looking to Build Your Next Web Project?
              </h2>
              <p className="text-xs sm:text-sm text-foreground/70 max-w-lg mx-auto leading-relaxed mb-6">
                Discuss your business goals and custom technical requirements with the KreativLabs team.
              </p>
